...
io.on('connection', function (socket) {
    ...
    socket.on('{{env('SOCKET_PREFIX' , '')}}auto_bid_updates', function (data) {
        io.emit('{{env('SOCKET_PREFIX' , '')}}{{env('SOCKET_PREFIX' , '')}}auto_bid_updates', { "loser": data.loser, "paddleNo": data.paddleNo, "latestAutoBidId": data.latestAutoBidId, "bidID": data.bidID, "autobidamount": data.autobidamount, "id": data.id, "user_id": data.user_id, "totalAutoBidLiability": data.totalAutoBidLiability, "outbid": data.outbid, "autobidUserID": data.autobidUserID, "bid_amountNew": data.bid_amountNew, "nextIncrement": data.nextIncrement, "liability": data.liability, "winneruser": data.winneruser, "checkTimer": data.checkTimer, "isgroup": data.isgroup, "groupUsers": data.groupUsers, "groupPaddleNo": data.groupPaddleNo });
    });
    socket.on('{{env('SOCKET_PREFIX' , '')}}auto_bid_delete', function (data) {
        io.emit('{{env('SOCKET_PREFIX' , '')}}{{env('SOCKET_PREFIX' , '')}}auto_bid_delete', { "autobidamount": data.autobidamount, "auction_product_id": data.auction_product_id });
    });
    socket.on('{{env('SOCKET_PREFIX' , '')}}add_bid_updates', function (data) {
        io.emit('{{env('SOCKET_PREFIX' , '')}}{{env('SOCKET_PREFIX' , '')}}add_bid_updates', {
            "loser": data.loser,
    ...
    socket.on('{{env('SOCKET_PREFIX' , '')}}auto_bid_update_user_amount', function (data) {
        io.emit('{{env('SOCKET_PREFIX' , '')}}{{env('SOCKET_PREFIX' , '')}}auto_bid_update_user_amount', { "autobidamount": data.autobidamount, "id": data.id, "user_id": data.user_id });
    });
    socket.on('{{env('SOCKET_PREFIX' , '')}}add_auction_status', function (data) {
        io.emit('{{env('SOCKET_PREFIX' , '')}}{{env('SOCKET_PREFIX' , '')}}add_auction_status', { "auctionstatus": data.auctionstatus });
    });
    socket.on('{{env('SOCKET_PREFIX' , '')}}publish_winner', function (data) {
        io.emit('{{env('SOCKET_PREFIX' , '')}}{{env('SOCKET_PREFIX' , '')}}publish_winner', { "publish_winner": 1 });
    });
    socket.on('{{env('SOCKET_PREFIX' , '')}}add_timer_reset', function (data) {
        io.emit('{{env('SOCKET_PREFIX' , '')}}{{env('SOCKET_PREFIX' , '')}}add_timer_reset', { "timerreset": data.timerreset });
    });
    socket.on('{{env('SOCKET_PREFIX' , '')}}end_of_auction_timer', function (data) {
        io.emit('{{env('SOCKET_PREFIX' , '')}}{{env('SOCKET_PREFIX' , '')}}end_of_auction_timer', { "timer": data.timer });
    });
    socket.on('{{env('SOCKET_PREFIX' , '')}}add_groupbid_updates', function (data) {
        io.emit('{{env('SOCKET_PREFIX' , '')}}{{env('SOCKET_PREFIX' , '')}}add_groupbid_updates', { "offersdata": data.offersdata, "adminofferData": data.adminofferData });
    });
    socket.on('{{env('SOCKET_PREFIX' , '')}}auction_timer', function (data) {
        io.emit('{{env('SOCKET_PREFIX' , '')}}{{env('SOCKET_PREFIX' , '')}}auction_timer', { "minutes":data.minutes,"seconds":data.seconds});
     });
